---
id: penzilla
name: Penzilla
url: https://penzilla.itch.io
category: 2d
subcategories: [sprites, ui, pixel, icons]
license: custom
commercial: true
attribution_required: unknown
formats: [PNG]
tags: [pixel, gui, itch]
verified: 2026-08-24
status: needs-review
---

# Penzilla

Penny / Penzilla Design: mixed free and paid pixel interiors, GUI, icons, and character sheets on itch. Hub copy says royalty-free game assets. Free listings on 2026-08-24 (Giant Basic GUI Bundle, Hooded Protagonist, Top-Down Retro Interior) all say "Royalty free and commercial use" and attach `PenzillaDesign_StandardLicense.pdf`. That PDF was not readable without an itch download, and none of those pages expose an itch Asset license field. Do not treat this store as CC0.

## Notes

- Paid packs sit in the same account (Colorful Dungeon, food icons, master collection). Free-page terms do not automatically cover paid SKUs
- "Once purchased (see also licence)" appears even on name-your-price / free listings. Read the PDF in the zip before shipping
- Hooded Protagonist is a 32x32 pixel character with eight animations. Interior pack is top-down furniture, not a full outdoor tileset
- itch marketplace EULA is extra. This entry is the publisher's own Standard License, which we could not quote

## Evidence

- Live pack pages (2026-08-24): "Royalty free and commercial use"
- Same pages (2026-08-24): download includes `PenzillaDesign_StandardLicense.pdf`. File body not fetched on 2026-08-24
- No itch "Asset license" CC0/CC-BY field on the three free listings checked. Stay `needs-review` until the PDF is quoted

## Related

- [pixel-frog](pixel-frog.md)
- [ansimuz-sunnyland](ansimuz-sunnyland.md)
- [kenney-ui-pack](kenney-ui-pack.md)
- [craftpix](craftpix.md)
