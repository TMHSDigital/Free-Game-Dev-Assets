---
id: luizmelo-monsters-creatures-fantasy
name: LuizMelo Monsters Creatures Fantasy
url: https://luizmelo.itch.io/monsters-creatures-fantasy
publisher: LuizMelo
category: 2d
subcategories: [sprites, characters]
license: CC0
license_spdx: CC0-1.0
commercial: true
attribution_required: false
formats: [PNG]
tags: [pixel, platformer, spritesheet]
camera_perspective: side_scroller
verified: 2026-08-24
status: active
---

# LuizMelo Monsters Creatures Fantasy

CC0 2D animated enemy pack (Skeleton, Mushroom, Goblin, Flying Eye) with idle/walk/attack/hit/death sheets. Ready for side-view action/platformers.

## Notes

- Credits are not required, but greatly appreciated
- PNG sheets; four creatures, each with a small frame set

## Evidence

- Live itch page (2026-08-24): “can be used in commercial and non-commercial projects… License: CC0 (Creative Commons Zero).”
- Same page (2026-08-24): Asset license "Creative Commons Zero v1.0 Universal"

## Related

- [luizmelo-martial-hero](luizmelo-martial-hero.md)
- [luizmelo-evil-wizard](luizmelo-evil-wizard.md)
- [pixel-frog](pixel-frog.md)
