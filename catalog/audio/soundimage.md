---
id: soundimage
name: Soundimage.org (Eric Matyas)
url: https://soundimage.org
category: audio
subcategories: [music, ambience]
license: custom
commercial: true
attribution_required: true
attribution_string: "Music by Eric Matyas www.soundimage.org"
formats: [MP3, WAV]
tags: [music, loops, game-oriented]
verified: 2026-08-24
status: active
---

# Soundimage.org

Loop-oriented music and atmospheres by Eric Matyas under a custom Soundimage International Public License (CC-BY-shaped: royalty-free share plus attribution; no NonCommercial clause). Credit must appear in the actual game, not only on a store listing. Ogg is the looping game format; MP3 is the non-looping film/podcast format.

## Notes

- License: [soundimage.org/sample-page/](https://soundimage.org/sample-page/) (About and License)
- Required credit format: [attribution-info](https://soundimage.org/attribution-info/)
- Homepage: Ogg for games, MP3 for film/video
- Formats field stays MP3/WAV as in the catalog schema; the live site emphasizes Ogg plus MP3

## Evidence

- Live homepage (2026-08-24): "Commercial use is fine, too, so no worries."
- Live About and License (2026-08-24): "worldwide, royalty-free, non-sublicensable, non-exclusive, irrevocable license"
- Live Attribution Info (2026-08-24): "credit me in your actual game, video, interactive novel, podcast, etc."
- Same page (2026-08-24): "Music by Eric Matyas www.soundimage.org"

## Related

- [incompetech](incompetech.md)
- [kenney-music-jingles](kenney-music-jingles.md)
- [purple-planet](purple-planet.md)
