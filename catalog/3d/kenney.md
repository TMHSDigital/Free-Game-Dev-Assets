---
id: kenney
name: Kenney
url: https://kenney.nl
publisher: Kenney
category: 3d
subcategories: [models, environments, ui, audio]
license: CC0
commercial: true
attribution_required: false
formats: [FBX, OBJ, glTF, PNG]
tags: [low-poly, modular, beginner-friendly, huge-library]
verified: 2026-08-25
status: active
---

# Kenney

Thousands of modular low-poly 3D kits, 2D sheets, UI, and audio packs. Per-pack pages carry a Creative Commons CC0 license field. Consistent grid sizing makes prototyping fast across engines. Preferred over anonymous "CC0" dumps because each pack page states the license.

## Notes

- Homepage copy is "completely free game assets"; CC0 is on the per-pack License field, not the hub
- Paid Kenney tools and the all-in-one itch bundle are separate from the free pack pages
- Space Kit and Industrial Kit are strong for tech/server-room props and flat-shaded pipelines
- 2D Input Prompts (64x64) are catalogued separately for Switch 2 / Quest / Deck coverage
- This hub stays under `3d` as the flagship library; 2D and audio packs have their own entries

## Evidence

- Live pack page [City Kit (Roads)](https://kenney.nl/assets/city-kit-roads) (2026-08-25): License field "Creative Commons CC0"
- Live homepage (2026-08-24): "Thousands of completely free game assets for you to use." (not a license grant)

## Related

- [kenney-city-kit-roads](kenney-city-kit-roads.md)
- [kenney-city-kit-commercial](kenney-city-kit-commercial.md)
- [kenney-city-kit-suburban](kenney-city-kit-suburban.md)
- [kenney-city-kit-industrial](kenney-city-kit-industrial.md)
- [kenney-furniture-kit](kenney-furniture-kit.md)
- [kenney-factory-kit](kenney-factory-kit.md)
- [kenney-food-kit](kenney-food-kit.md)
- [kenney-graveyard-kit](kenney-graveyard-kit.md)
- [kenney-fantasy-town-kit](kenney-fantasy-town-kit.md)
- [kenney-modular-space-kit](kenney-modular-space-kit.md)
- [kenney-train-kit](kenney-train-kit.md)
- [kenney-pirate-kit](kenney-pirate-kit.md)
- [kenney-modular-dungeon-kit](kenney-modular-dungeon-kit.md)
- [kenney-modular-cave-kit](kenney-modular-cave-kit.md)
- [kenney-mini-forest](kenney-mini-forest.md)
- [kenney-mini-skate](kenney-mini-skate.md)
- [kenney-mini-dungeon](kenney-mini-dungeon.md)
- [kenney-mini-arena](kenney-mini-arena.md)
- [kenney-platformer-kit](kenney-platformer-kit.md)
- [kenney-castle-kit](kenney-castle-kit.md)
- [kenney-car-kit](kenney-car-kit.md)
- [kenney-nature-kit](kenney-nature-kit.md)
- [quaternius](quaternius.md)
- [kaykit](kaykit.md)
- [kenney-input-prompts](../2d/kenney-input-prompts.md)
- [kenney-rpg-audio](../audio/kenney-rpg-audio.md)
